...
var (
	taskDBTypes = map[string]string{`ID`: `integer`, `Title`: `character varying`, `Description`: `text`, `Color`: `character varying`, `StartsAt`: `timestamp without time zone`, `DoneAt`: `timestamp without time zone`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
